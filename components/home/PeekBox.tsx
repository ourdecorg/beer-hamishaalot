...
export default function PeekBox({ lang }: Props) {
  const tr  = t(lang).peekBox
  const rtl = lang === 'he'
  ...
  useEffect(() => {
    if (boxState === 'loading') {
      timerRef.current = setInterval(
        () => setMsgIdx(i => (i + 1) % tr.loadingMessages.length),
        600,
      )
    } else {
  ...
  }, [boxState, tr.loadingMessages.length])

  async function handleSubmit() {
    const trimmed = input.trim()
    if (trimmed.length < MIN_LENGTH) {
      setError(tr.errorShort)
      return
    }
    setError(null)
    setBoxState('loading')
  ...
  async function handleSendNote(wishId: string) {
    if (!noteForm.message.trim()) {
      setNoteError(tr.noteValidationError)
      return
    }
    setNoteSending(true)
    setNoteError(null)
    try {
      ...
      setNoteError(tr.noteGenericError)
    } finally {
  ...
  if (boxState === 'empty') {
    return (
      <div className="text-center space-y-4 py-6" dir={dir}>
        <p className="text-slate-500">{tr.emptyMessage}</p>
        <button
          onClick={handleReset}
  ...
  // ── Results ────────────────────────────────────────────────────────────────
  return (
    <div className={`flex flex-col gap-5 ${textAlign}`} dir={dir}>
      <p className="text-base font-medium text-slate-700">{tr.resultsHeading}</p>

      <div className="flex flex-col gap-4">
        {results.map(r => {
          const sent     = noteSentIds.has(r.wish_id)
          const noteOpen = noteOpenFor === r.wish_id
          const collabLabel = r.collaboration_type
            ? (tr.collabLabels[r.collaboration_type as keyof typeof tr.collabLabels] ?? r.collaboration_type)
            : null
          ...
          return (
            ...
            >
              ...
              {(collabLabel || r.emotional_tone) && (
                <div className="flex gap-2 flex-wrap">
                  {collabLabel && (
                    <span className="tag-badge text-xs">{collabLabel}</span>
                  )}
                  {r.emotional_tone && (
              ...
              {sent ? (
                ...
                <div className="flex flex-col gap-2 pt-1 border-t border-slate-100">
                  <p className="text-xs font-semibold text-slate-500 mb-1">{tr.noteFormTitle}</p>
