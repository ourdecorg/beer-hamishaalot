...
describe('computeMatchScore', () => {
  ...
  test('formula weights: 0.35×semantic + 0.30×comp + 0.15×intent + 0.20×structural', () => {
    expect(computeMatchScore(1, 0, 0, 0).match_score).toBeCloseTo(0.35, 5)
    expect(computeMatchScore(0, 1, 0, 0).match_score).toBeCloseTo(0.30, 5)
    expect(computeMatchScore(0, 0, 1, 0).match_score).toBeCloseTo(0.15, 5)
    expect(computeMatchScore(0, 0, 0, 1).match_score).toBeCloseTo(0.20, 5)
  })

  test('weights sum to 1.0', () => {
    expect(computeMatchScore(1, 1, 1, 1).match_score).toBeCloseTo(1.0, 5)
  })
})
